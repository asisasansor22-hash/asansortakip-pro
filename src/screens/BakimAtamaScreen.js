...
export default function BakimAtamaScreen({ data }) {
  ...
  const atananMap = useMemo(() => {
    const m = new Map();
    ayBakimlari.forEach((b) => {
      if (b.durum === 'atandi' && !yapilanMap.has(b.asansorId)) m.set(b.asansorId, b);
    });
    return m;
  ...
  const ilceler = useMemo(() => {
    ...
    return Object.keys(grouped).sort().map((ilce) => {
      ...
      return { ilce, items, toplam: items.length, yapilan, atanan, kalan: items.length - yapilan - atanan };
    });
  ...
  const toplamYapilan = yapilanMap.size;
  const toplamAtanan = atananMap.size;

  const toggleSecim = (id) => {
    setSeciliIds((p) => p.includes(id) ? p.filter((x) => x !== id) : [...p, id]);
  };

  const tumunuSec = (items) => {
    const ids = items.map((e) => e.id);
    const hepsiSecili = ids.every((id) => seciliIds.includes(id));
    if (hepsiSecili) {
      const set = new Set(ids);
      setSeciliIds((p) => p.filter((x) => !set.has(x)));
    } else {
      setSeciliIds((p) => [...new Set([...p, ...ids])]);
    }
  };

  const topluBakimciAta = (bakimci) => {
    ...
    setModalAcik(false);
    setSeciliIds([]);
    Alert.alert('Atama Yapıldı', `${seciliIds.length} asansör ${bakimci.ad} bakımcısına atandı.`);
  };
  ...
  const getBakimci = (id) => (bakimcilar || []).find((b) => b.id === id) || null;

  const renderBakimciModal = () => (
    <SheetModal
      visible={modalAcik}
      onClose={() => setModalAcik(false)}
      title={`Bakımcı Ata (${seciliIds.length} asansör)`}
    >
      {(bakimcilar || []).length === 0 ? (
        <Empty text="Bakımcı kaydı yok." />
      ) : (
        <View style={{ paddingBottom: 10 }}>
          {(bakimcilar || []).map((b) => (
            <TouchableOpacity
              key={b.id}
              style={styles.bakimciSecenek}
              onPress={() => topluBakimciAta(b)}
              activeOpacity={0.7}
            >
              <View style={[styles.avatar, { backgroundColor: b.renk || '#3b82f6' }]}>
                <Text style={styles.avatarText}>{(b.ad || '?')[0].toUpperCase()}</Text>
              </View>
  ...
  if (acilanIlce && ilceDetay) {
    ...
    const ilceSeciliCount = ilceDetay.items.filter((e) => seciliIds.includes(e.id)).length;

    return (
      <View style={styles.container}>
        <View style={styles.detayHeader}>
          <TouchableOpacity style={styles.backBtn} onPress={() => setAcilanIlce(null)} activeOpacity={0.7}>
            <Text style={styles.backBtnText}>‹ Geri</Text>
          </TouchableOpacity>
          <View style={styles.flex1}>
            <Text style={[styles.detayTitle, { color: renk }]}>{acilanIlce}</Text>
            <Text style={styles.detaySub}>
              {MONTHS[seciliAy]} {seciliYil} · {ilceDetay.yapilan}/{ilceDetay.toplam} yapıldı
            </Text>
          </View>
          <TouchableOpacity
            style={[styles.tumuSecBtn, { backgroundColor: renk + '25' }]}
            onPress={() => tumunuSec(ilceDetay.items)}
          >
            <Text style={[styles.tumuSecText, { color: renk }]}>
              {ilceSeciliCount === ilceDetay.items.length ? 'Temizle' : 'Tümünü Seç'}
            </Text>
          </TouchableOpacity>
        </View>
        ...
        <ScrollView contentContainerStyle={styles.list}>
          {sirali.map((gun) => {
            const items = gunGruplari[gun].slice().sort((a, b) => {
              const as = yapilanMap.has(a.id) ? 2 : atananMap.has(a.id) ? 1 : 0;
              const bs = yapilanMap.has(b.id) ? 2 : atananMap.has(b.id) ? 1 : 0;
              ...
              <View key={gun} style={styles.gunGrup}>
                <View style={[styles.gunHeader, { backgroundColor: renk + '15', borderColor: renk + '40' }]}>
                  ...
                  <Text style={[styles.gunTitle, { color: renk }]}>
                    {gun === '—' ? 'Gün belirlenmemiş' : 'Her ayın ' + gun + '. günü'}
                  </Text>
                  <View style={[styles.gunCount, { backgroundColor: renk + '30' }]}>
                    <Text style={[styles.gunCountText, { color: renk }]}>{gunYapilan}/{items.length}</Text>
                  </View>
                </View>
                {items.map((e) => {
        ...
        {seciliIds.length > 0 ? (
          <View style={styles.floatBar}>
            <Text style={styles.floatText}>{seciliIds.length} seçili</Text>
            <TouchableOpacity style={styles.floatBtn} onPress={() => setModalAcik(true)}>
              <Text style={styles.floatBtnText}>Bakımcı Ata</Text>
            </TouchableOpacity>
            <TouchableOpacity style={styles.floatClear} onPress={() => setSeciliIds([])}>
              <Text style={styles.floatClearText}>✕</Text>
            </TouchableOpacity>
          </View>
        ) : null}
  ...
  return (
    <View style={styles.container}>
      <View style={styles.ayHeader}>
        ...
        <Text style={styles.ayText}>{MONTHS[seciliAy]} {seciliYil}</Text>
        <TouchableOpacity style={styles.ayBtn} onPress={() => ayDegistir(1)}>
          <Text style={styles.ayBtnText}>›</Text>
        </TouchableOpacity>
      </View>
      ...
      <View style={styles.statRow}>
        <View style={styles.statBox}>
          <Text style={[styles.statValue, { color: '#34c759' }]}>{toplamYapilan}</Text>
          <Text style={styles.statLabel}>Yapılan</Text>
        </View>
        <View style={styles.statBox}>
          <Text style={[styles.statValue, { color: '#007AFF' }]}>{toplamAtanan}</Text>
          <Text style={styles.statLabel}>Atanan</Text>
        </View>
        <View style={styles.statBox}>
          <Text style={[styles.statValue, { color: '#f59e0b' }]}>{elevs.length - toplamYapilan - toplamAtanan}</Text>
      ...
      <ScrollView contentContainerStyle={styles.list}>
        {ilceler.length === 0 ? (
          ...
          ilceler.map((g) => {
            ...
            const pct = g.toplam > 0 ? Math.round((g.yapilan / g.toplam) * 100) : 0;
            return (
              <TouchableOpacity key={g.ilce} style={styles.ilceCard} onPress={() => setAcilanIlce(g.ilce)} activeOpacity={0.7}>
                <View style={[styles.ilceBar, { backgroundColor: renk }]} />
                <View style={styles.flex1}>
                  <View style={styles.ilceTopRow}>
                    <Text style={[styles.ilceTitle, { color: renk }]}>{g.ilce}</Text>
                    <Text style={styles.ilceToplam}>{g.toplam} bina</Text>
                  </View>
                  <View style={styles.progressBarBg}>
                    <View style={[styles.progressFill, { width: pct + '%', backgroundColor: renk }]} />
                  </View>
                  <View style={styles.ilceStatRow}>
                    <Text style={[styles.statChip, { color: '#34c759' }]}>✓ {g.yapilan}</Text>
                    <Text style={[styles.statChip, { color: '#007AFF' }]}>◉ {g.atanan}</Text>
                    <Text style={[styles.statChip, { color: '#94a3b8' }]}>· {g.kalan}</Text>
...
const styles = StyleSheet.create({
  ...
  ayHeader: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', padding: 16, gap: 16 },
  ayBtn: { width: 40, height: 40, borderRadius: 20, backgroundColor: '#1c1e2a', alignItems: 'center', justifyContent: 'center' },
  ...
  statBox: { flex: 1, backgroundColor: '#1c1e2a', borderRadius: 12, padding: 12, alignItems: 'center', borderWidth: 0.5, borderColor: '#2a3050' },
  statValue: { fontSize: 20, fontWeight: '800' },
  statLabel: { fontSize: 11, color: '#94a3b8', marginTop: 2 },
  sectionLabel: { paddingHorizontal: 16, paddingBottom: 8, fontSize: 11, color: '#64748b', fontWeight: '700', letterSpacing: 0.5 },
  list: { padding: 16, paddingTop: 4, paddingBottom: 100 },
  ilceCard: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#1c1e2a', borderRadius: 14, marginBottom: 10, gap: 12, borderWidth: 0.5, borderColor: '#2a3050', overflow: 'hidden' },
  ilceBar: { width: 4, alignSelf: 'stretch' },
  flex1: { flex: 1, padding: 12, paddingLeft: 8 },
  ilceTopRow: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 },
  ilceTitle: { fontSize: 16, fontWeight: '800' },
  ilceToplam: { fontSize: 12, color: '#94a3b8' },
  progressBarBg: { height: 5, backgroundColor: '#2a2d3a', borderRadius: 3, overflow: 'hidden', marginBottom: 8 },
  progressFill: { height: '100%', borderRadius: 3 },
  ilceStatRow: { flexDirection: 'row', alignItems: 'center', gap: 12 },
  statChip: { fontSize: 12, fontWeight: '700' },
  yuzdeText: { marginLeft: 'auto', fontSize: 12, color: '#94a3b8', fontWeight: '600' },
  chevron: { fontSize: 22, color: '#64748b', paddingRight: 12 },
  detayHeader: { flexDirection: 'row', alignItems: 'center', padding: 14, gap: 10, borderBottomWidth: 0.5, borderBottomColor: '#1e2236' },
  ...
  tumuSecBtn: { paddingHorizontal: 12, paddingVertical: 7, borderRadius: 10 },
  tumuSecText: { fontSize: 12, fontWeight: '800' },
  gunGrup: { marginBottom: 14 },
  gunHeader: { flexDirection: 'row', alignItems: 'center', gap: 8, paddingHorizontal: 12, paddingVertical: 10, borderRadius: 10, borderWidth: 0.5, marginBottom: 6 },
  gunEmoji: { fontSize: 14 },
  gunTitle: { flex: 1, fontSize: 13, fontWeight: '800' },
  gunCount: { paddingHorizontal: 9, paddingVertical: 3, borderRadius: 10 },
  ...
  floatBtnText: { color: '#fff', fontWeight: '800', fontSize: 13 },
  floatClear: { width: 36, height: 36, borderRadius: 10, backgroundColor: '#ff3b3020', alignItems: 'center', justifyContent: 'center' },
  floatClearText: { color: '#ff3b30', fontSize: 16, fontWeight: '900' },
  bakimciSecenek: { flexDirection: 'row', alignItems: 'center', padding: 12, gap: 12, borderRadius: 12, marginBottom: 6, borderWidth: 1, borderColor: '#2a3050', backgroundColor: '#1a1d28' },
